function Params=SimAnn(InputParams,Data,r_vect) 
% This function runs the simulated annealing based fitting process on input
% QSO data stored in matrix 'Data'. The number of dimensions of parameter
...
% deviations between data and the reference model, a.k.a. the chi-squared 
% statistic) within the boundaries of the parameter space. To do this, we 
% start a wandering process at a randomly chosen allowed point of parameter
% space, whose coordinates are randomized from uniform distributions 
% defined between the boundaries of individual parameters. We then lower
% the value of a temperature parameter 'T' with each step in the wandering,
% from 'T_start' (which is set to be the measure of goodness of fit at the 
% starting point) to T_crit=0.01*T_start by a factor of (1-T_step), where 
% T_step=0.001. The wandering finishes when T<=T_crit, and then the final 
% set of parameters (coordinates of the point where the wandering 
% terminates) as well as corresponding measure of goodness of fit is output
% in vector 'Params'. Each step of the wandering starts with randomly 
% choosing a parameter to change (i.e. the axis which we step along), and 
% randomizing a step length from a Gaussian distribution with zero mean and
% sigma equal to the value set in the third column of 'InputParams'. If the
% step moves us out from the interval of allowed parameters, we step toward 
% the opposite direction along the axis of the same parameter (in the 
% exceptional case when this also moves us out from the boundaries - which 
% means that the step length in InputParams was set to be too large -, then
% we terminate the whole fitting process, and display an error message on 
% the screen). The step is accepted with a probability of 1 if the measure
% of goodness of fit is smaller than it was before, and it is accepted with
% a probability of p=exp(-Diff/T) if the measure is larger, where 'Diff' is
% the absolute difference between the old and the new measure (one can see
% that as T decreases with every step, the same Diff results with a
% decreasing probability of the new position being accepted).
%
%
% Inputs for SimAnn:
% 
...
% with it.
OutStat=TimeSphereFitter(Params,Data,r_vect);
T_start=OutStat;
...
while(T>T_crit)
    ...
    if((Test_Params(ParamInd)>=InputParams(ParamInd,1))&(Test_Params(ParamInd)<=InputParams(ParamInd,2)))            
        Test_OutStat=TimeSphereFitter(Test_Params,Data,r_vect);        
    else        
